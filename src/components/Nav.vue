        ...
        <div class="relative">
            <div class="relative overflow-x-hidden transition-all duration-200"
                ...
                <div class="relative">
                    ...
                    <span class="absolute top-1/2 left-3 transform -translate-y-1/2 text-lg pl-1">{{ $store.state.currency }}</span>
                </div>
            </div>
            <aside :class="activeFunction === 'monthSavings' ? 'w-auto' : 'w-0 hidden'" class="bg-red-500 text-white p-1 px-2 text-xs -bottom-8 absolute left-0 transition-transform duration-200">{{ $store.state.currency }}{{ rentCalcValue }} max</aside>
        </div>
        <div class="flex items-center">
            <button class="flex bg-zinc-500 self-end rounded-full w-14 h-8 relative" @click="toggleCurrency">
                <div :class="isMXN ? 'translate-x-7 rotate-180 mr-1 bg-mexico' : 'translate-x-0 left-0 ml-1 bg-unitedKingdom'"
                    class="absolute w-6 h-6 rounded-full top-1/2 -translate-y-1/2 transition-all duration-200 dark:bg-zinc-950 bg-repeat-round">
                </div>
            </button>
        </div>
        <div class="flex items-center">
